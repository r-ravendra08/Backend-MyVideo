...
import com.myVideo.exception.ResourceNotFound;
import com.myVideo.repository.VideoRepository;
...
public class VideoService implements VideoInterface {
	...
	public Videos getVideosById(Integer id) {
		Videos video = this.videoRepository.findById(id)
				.orElseThrow(() -> new ResourceNotFound("504", "id is not present"));
		return video;
	}
	...
	public void deleteVideos(Integer id) {
		Videos video = this.videoRepository.findById(id)
				.orElseThrow(() -> new ResourceNotFound("403", "video id not found"));
